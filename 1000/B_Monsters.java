...
public class B_Monsters {
    ...
    static void solve() throws IOException {
        ...
        for(int i=0; i<n; i++) {
            int x = nextInt();
            set.add(new Pair<>(i+1, x%k));
        }

        // out.println(set);
        Iterator<Pair<Integer, Integer>> ite = set.iterator();
        while(ite.hasNext()) {
            var e = ite.next();
            out.print(e.x + " ");
        }
...
final class Pair<T1, T2> implements Comparable<Pair<T1, T2>> {
    ...
    public int compareTo(Pair<T1, T2> other) {
        if(Integer.compare((int)this.y, (int)other.y) != 0) {
            if((int)this.y <= 0) return -1;
            else if((int)other.y <= 0) return 1;

            return -Integer.compare((int)this.y, (int)other.y);
        }
        else {
