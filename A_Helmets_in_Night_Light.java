...
public class A_Helmets_in_Night_Light {
    ...
    static void solve() throws IOException {
        ...
        for(int i=0; i<n; i++) {
            int x = nextInt();
            if(x<k)
                map.put(x, map.getOrDefault(x, 0L)+arr[i]);
        }

        List<Map.Entry<Integer, Long>> li = new ArrayList<>(map.entrySet());

        int i = 0;
        long cost = k;
        n--;
        while(n>0 && i<li.size()) {
            Map.Entry<Integer, Long> e = li.get(i);
            long mini = Math.min(n, e.getValue());
            cost += e.getKey()*mini;
            n-=mini;
            i++;
        }
